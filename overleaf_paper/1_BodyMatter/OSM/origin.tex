...
%=================================================================
\documentclass[remotesensing,article,accept,moreauthors,pdftex,10pt,a4paper]{mdpi} 
...
\history{Received: 12 March 2018 ; Accepted: 16 April 2018; Published: 18 April 2018}
%------------------------------------------------------------------
...
% \definecolor{myroad}{RGB}{128,64,128}
...
%\Title{Optimization of OpenStreetMap building footprints based on semantic segmentation with deep learning of UAV oblique images}
\Title{Optimization of OpenStreetMap Building Footprints~Based on Semantic Information \mbox{of Oblique UAV Images}}

% Authors, for the paper (add full first names)
\Author{Xiangyu Zhuo %Please carefully check the accuracy of names and affiliations.
$^{1, 2,}$*, Friedrich Fraundorfer $^{1,3}$ , Franz Kurz $^{1}$ and Peter Reinartz $^{1}$}
% Authors, for metadata in PDF
\AuthorNames{Xiangyu Zhuo, Friedrich Fraundorfer, Franz Kurz and Peter Reinartz}

% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.)
\address{%
$^{1}$ \quad Remote Sensing Technology Institute, German Aerospace Center, 82234 Wessling, Germany; fraundorfer@icg.tugraz.at (F.F.); franz.kurz@dlr.de (F.K.); Peter.Reinartz@dlr.de (P.R.)\\
$^{2}$ \quad Remote Sensing Technology, Technische Universit\"at M\"unchen, 80333 Munich, Germany\\
$^{3}$ \quad Institute for Computer Graphics and Vision, Graz University of Technology, 8010 Graz, Austria}

% Contact information of the corresponding author
\corres{Correspondence: xiangyu.zhuo@dlr.de; Tel.: +49-8153-28-4235 }
...
\abstract{\textls[-20]{Building footprint information is vital for 3D building modeling. Traditionally, in remote sensing, building footprints are extracted and delineated from aerial imagery and/or LiDAR point cloud. Taking a different approach, this paper is dedicated to the optimization of OpenStreetMap (OSM) building footprints exploiting the contour information, which is derived from deep learning-based semantic segmentation of oblique images acquired by the Unmanned Aerial Vehicle (UAV). First, a simplified 3D building model of Level of Detail 1 (LoD 1) is initialized using the footprint information from OSM and the elevation information from Digital Surface Model (DSM). In parallel, a deep neural network for pixel-wise semantic image segmentation is trained in order to extract the building boundaries as contour evidence. Subsequently, an optimization integrating the contour evidence from multi-view images as a constraint results in a refined 3D building model with optimized footprints and height. Our method is leveraged to optimize OSM building footprints for four datasets with different building types, demonstrating robust performance for both individual buildings and multiple buildings regardless of image resolution. Finally, we compare our result with reference data from German Authority Topographic-Cartographic Information System (ATKIS). Quantitative and qualitative evaluations reveal that the original OSM building footprints have large offset, but can be significantly improved from meter level to decimeter level after optimization.}}



%\abstract{With a global cast of volunteers, OpenStreetMap (OSM) has gained increasing popularity as it provides free and updated geographic information. One application is reconstructing three-dimensional (3D) building models based on the building footprints in OSM database, which are mostly generated from the imagery of Bing map. Therefore the quality of 3D building modeling depends strongly on the geo-localization accuracy of the Bing map imagery, and the average offset is about 4 meters \cite{fan2014quality}. In this paper, we propose a novel approach to improve the accuracy of OSM foot prints based on semantic segmentation of geo-registered UAV images. This method consists of three steps: first, geo-register the UAV images with high-accuracy aerial imagery; second, classify the UAV images using a Fully Convolutional Network (FCN) for semantic segmentation, and then extract the boundary between building and ground; finally, adjust the OSM footprints to make the edges of OSM footprints fit  best with the more realistic boundary. A series of experiments involving different scenarios are conducted to validate the proposed method, and the results demonstrate that our methods works not only for optimization of a single building, but also for multiple buildings and low-resolution images. when comparing to the ATKIS (German Authority Topographic–Cartographic Information System) data.}%

% Keywords
\keyword{building footprint; oblique UAV images; semantic segmentation; deep neural network}
...
a,       OSM,     Optimized
1,       0.000004824,          0.000004824
2,       0.000004687,           0
3,       0.000009425,          0
4,       0.000004794,        0.000004794
5,       0.000004565,          0
\end{filecontents*}
...
\textls[-15]{Given an initial hypothesis of the building boundary, the refinement with well-designed constraints plays a vital role in improving the accuracy of building footprints. The constraints usually come from the 3D features embedded in DSMs 
%Define if appropriate
...
\textls[-15]{Another challenge is the accurate geo-registration of UAV image blocks. Although UAV images are usually coupled with on-board GNSS/INS information, their absolute
%Define if appropriate
...
\textls[-20]{We solve the minimization problem using a modification of Powell’s method \cite{powell1964efficient, press1989numerical}, which performs sequential one-dimensional minimizations along each vector of the directions set. After optimization, the~accuracy of the building footprint and height improves. }
...
\begin{figure}[H]
    \centering
       ...
       \end{subfigure}
       \vspace{-12pt}
\caption{\textls[-35]{Overview of optimizable building vertices in the presence of occlusions. Red lines are the projection of original OSM building footprints before optimization, highlighting the building edges that can be optimized.}}
...
\textls[-25]{We used the software \hl{Pix4Dmapper Pro (version 4.0.25)} for the process and orientation of the UAV data. The mean reprojection errors of the four datasets are in the range of 0.15--0.2 pixels.} 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\acknowledgments{\hl{This research was funded by the German Academic Exchange Service (DAAD:DLR/DAAD Research Fellowship Nr. 50019750) for Xiangyu Zhuo.}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\authorcontributions{Friedrich Fraundorfer and Xiangyu Zhuo conceived and designed the experiments; Xiangyu Zhuo performed the experiments and analyzed the data; Franz Kurz contributed to the data acquisition and pre-processing; Xiangyu Zhuo wrote the manuscript and all authors contributed to reviewing the paper.}

\conflictsofinterest{\hl{The authors declare no conflict of interest.}}

\abbreviations{\hl{The following abbreviations are used in this manuscript:}\\
\noindent 
\begin{tabular}{@{}ll}
...
\bibitem[Goetz(2013)]{Goetz2012towards}
Goetz, M.
...
\newblock {\em Int. J. Geogr. Inf.~Sci.} {\bf
  2013}, {\em 27},~845--865,
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.1080/13658816.2012.721552}{\detokenize{10.1080/13658816.2012.721552}}}.

\bibitem[Fan \em{et~al.}(2014)Fan, Zipf, Fu, and Neis]{fan2014quality}
Fan, H.; Zipf, A.; Fu, Q.; Neis, P.
\newblock Quality assessment for building footprints data on OpenStreetMap.
\newblock {\em Int. J. Geogr. Inf. Sci.} {\bf
  2014}, {\em 28},~700--719,
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.1080/13658816.2013.867495}{\detokenize{10.1080/13658816.2013.867495}}}.

\bibitem[M{\"u}ller and Wilhelm~Zaum(2005)]{mueller2005robust}
M{\"u}ller, S.; Wilhelm~Zaum, D.
\newblock Robust building detection in aerial images. \emph{Int. Arch. Photogramm. Remote Sens.} {\bf 2005},
\newblock {\em 36}, 143--148.

\bibitem[Zhou and Neumann(2010)]{zhou20102}
Zhou, Q.Y.; Neumann, U.
\newblock 2.5 d dual contouring: A robust approach to creating building models
...
\bibitem[Lafarge and Mallet(2012)]{lafarge2012creat}
Lafarge, F.; Mallet, C.
\newblock Creating large-scale city models from 3D-point clouds: A robust
  approach with hybrid representation.
\newblock {\em Int. J. Comput. Vis.} {\bf 2012}, {\em
  99},~69--85,
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.1007/s11263-012-0517-8}{\detokenize{10.1007/s11263-012-0517-8}}}.

\bibitem[Xiao and Gerke(2015)]{xiao2015building}
Xiao, J.; Gerke, M.
\newblock Building footprint extraction based on radiometric and geometric
  constraints in airborne oblique images.
\newblock {\em Int. J. Image Data Fus.} {\bf 2015}, {\em
  6},~270--287.

\bibitem[Sirmacek and Unsalan(2008)]{Sirmacek2008building}
...
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.1109/ISCIS.2008.4717854}{\detokenize{10.1109/ISCIS.2008.4717854}}}.

\bibitem[Zhu \em{et~al.}(2015)Zhu, Jiang, and Zhang]{zhu2015feature}
Zhu, Q.; Jiang, W.; Zhang, J.
\newblock Feature line based building detection and reconstruction from oblique
  airborne imagery.
\newblock {\em Int. Arch. Photogramm. Remote Sens. Spat. Inf. Sci.} {\bf 2015}, {\em 40},~199.

\bibitem[Wegner \em{et~al.}(2009)Wegner, Thiele, and Soergel]{wegner2009fusion}
Wegner, J.D.; Thiele, A.; Soergel, U.
\newblock Fusion of optical and InSAR features for building recognition in
  urban areas.
\newblock {\em Int. Arch. Photogramm. Remote Sens. Spat. Inf. Sci.} {\bf 2009}, {\em 38},~W4.

\bibitem[Gevaert \em{et~al.}(2017)Gevaert, Persello, Sliuzas, and
  Vosselman]{gevaert2017informal}
...
\bibitem[Huang \em{et~al.}(2016)Huang, Cheng, Wang, Li, Shi, and
  Pan]{huang2016building}
...
\bibitem[M{\"u}ller \em{et~al.}(1998)M{\"u}ller, Seyfert, and
  Brandenburg]{muller1998quality}
...
\bibitem[Dornaika \em{et~al.}(2016)Dornaika, Moujahid, Merabet, and
  Ruichek]{DORNAIKA2016130}
...
\newblock {\em Expert Syst. Appl.} {\bf 2016}, {\em 58},~130--142.
\newblock
  

\bibitem[{Ok} \em{et~al.}(2013){Ok}, {Senaras}, and {Yuksel}]{ok2013exploiting}
{Ok}, A.O.; {Senaras}, C.; {Yuksel}, B.
\newblock {Exploiting shadow evidence and iterative graph-cuts for efficient
  detection of buildings in complex environments}.
\newblock {\em ISPRS Int. Arch. Photogramm. Remote Sens. Spat. Inf. Sci.} {\bf 2013}, \emph{XL-1/W1}, 269--274,
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.5194/isprsarchives-XL-1-W1-269-2013}{\detokenize{10.5194/isprsarchives-XL-1-W1-269-2013}}}.

\bibitem[Dini \em{et~al.}(2013)Dini, Jacobsen, and Heipke]{dini2013delineation}
...
\newblock {\em ISPRS Int. Arch. Photogramm. Remote Sens. Spat. Inf. Sci.} {\bf 2013}, {\em XL-1/W1},~81--85,
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.5194/isprsarchives-XL-1-W1-81-2013}{\detokenize{10.5194/isprsarchives-XL-1-W1-81-2013}}}.

\bibitem[Dai \em{et~al.}(2017)Dai, Gong, Li, and Feng]{dai2017building}
\textls[-35]{Dai, Y.; Gong, J.; Li, Y.; Feng, Q.
\newblock Building segmentation and outline extraction from UAV image-derived
  point clouds by a line growing algorithm.
\newblock {\em Int. J. Digit. Earth} {\bf 2017}, {\em
  10},~1077--1097,
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.1080/17538947.2016.1269841}{\detokenize{10.1080/17538947.2016.1269841}}}.}

\bibitem[Awrangjeb and Fraser(2014)]{awrangjeb2014automatic}
Awrangjeb, M.; Fraser, C.S.
\newblock Automatic segmentation of raw LIDAR data for extraction of building
  roofs.
\newblock {\em Remote Sens.} {\bf 2014}, {\em 6},~3716--3751,
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.3390/rs6053716}{\detokenize{10.3390/rs6053716}}}.

\bibitem[Bittner \em{et~al.}(2017)Bittner, Cui, and
  Reinartz]{bittner2017building}
Bittner, K.; Cui, S.; Reinartz, P.
\newblock Building extraction from remote sensing data using fully
  convolutional networks.
\newblock {\em Int. Arch. Photogramm. Remote Sens. Spat. Inf. Sci.} {\bf 2017}, {\em 42},~481.

\bibitem[Li \em{et~al.}(2013)Li, Wu, An, Xu, He, and Xu]{li2013fusion}
...
\newblock {\em Optik Int. J. Light Electron Opt.}
  {\bf 2013}, {\em 124},~5357--5362.



\bibitem[Nex \em{et~al.}(2013)Nex, Rupnik, and Remondino]{nex2013building}
...
\newblock {\em II-3/W3}, 61--66.

\bibitem[Hammoudi and Dornaika(2009)]{Hammoudi2009ExtractingBF}
...
\bibitem[Yang \em{et~al.}(2013)Yang, Wei, Li, and Li]{yang2013semiautomated}
Yang, B.; Wei, Z.; Li, Q.; Li, J.
\newblock Semiautomated building facade footprint extraction from mobile LiDAR
  point clouds.
\newblock {\em IEEE Geosci. Remote Sens. Lett.} {\bf 2013}, {\em
  10},~766--770,
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.1109/LGRS.2012.2222342}{\detokenize{10.1109/LGRS.2012.2222342}}}.

\bibitem[Vacca \em{et~al.}(2017)Vacca, Dessi, and Sacco]{vacca2017the}
Vacca, G.; Dessi, A.; Sacco, A.
\newblock The use of nadir and oblique UAV images for building knowledge.
\newblock {\em ISPRS Int. J. GeoInf.} {\bf 2017}, {\em
  6},~393.

\bibitem[LINGUA \em{et~al.}(2017)LINGUA, Noardo, Spano, Sanna, and
  Matrone]{lingua20173d}
...
\newblock {\em 42},~107--115.

\bibitem[{Aicardi} \em{et~al.}(2016){Aicardi}, {Chiabrando}, {Grasso},
  {Lingua}, {Noardo}, and {Span{\`o}}]{aicardi2016UAV}
{Aicardi}, I.; {Chiabrando}, F.; {Grasso}, N.; {Lingua}, A.M.; {Noardo}, F.;
  {Span{\`o}}, A.
\newblock {Uav photogrammetry with oblique images: First analysis on data
  acquisition and processing}.
\newblock {\em ISPRS Int. Arch. Photogramm. Remote Sens. Spat. Inf. Sci.} {\bf 2016}, \emph{41}, 835--842,
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.5194/isprs-archives-XLI-B1-835-2016}{\detokenize{10.5194/isprs-archives-XLI-B1-835-2016}}}.

\bibitem[Feifei \em{et~al.}(2012)Feifei, Zongjian, Dezhu, and
  Hua]{xie2012study}
...
\newblock {\em 39},~469--473.

\bibitem[Onyango \em{et~al.}(2017)Onyango, Nex, Peter, and
  Jende]{onyango2017accurate}
...
\newblock {\em Int. Arch. Photogramm. Remote Sens. Spat. Inf. Sci.} {\bf 2017}, {\em 42}, 599.

\bibitem[Zhuo \em{et~al.}(2017)Zhuo, Koch, Kurz, Fraundorfer, and
  Reinartz]{zhuo2017automatic}
...
\newblock {\em Remote Sens.} {\bf 2017}, {\em 9},~376.

\bibitem[Shelhamer \em{et~al.}(2017)Shelhamer, Long, and Darrell]{fcn}
Shelhamer, E.; Long, J.; Darrell, T.
\newblock Fully convolutional networks for semantic segmentation.
\newblock {\em IEEE Trans. Pattern Anal. Mach. Intell.}
  {\bf 2017}, {\em 39},~640--651.
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.1109/TPAMI.2016.2572683}{\detokenize{10.1109/TPAMI.2016.2572683}}}.

\bibitem[Zheng \em{et~al.}(2015)Zheng, Jayasumana, Romera-Paredes, Vineet, Su,
  Du, Huang, and Torr]{crfasrnn_ICCV2015}
...
\bibitem[Arnab \em{et~al.}(2016)Arnab, Jayasumana, Zheng, and
  Torr]{higherordercrf_ECCV2016}
...
\bibitem[Shih(2009)]{shih2009image}
Shih, F.Y.
\newblock {\em Image Processing and Mathematical Morphology: Fundamentals and
  Applications}; CRC Press: Boca Raton, FL, USA, 2009; 439p.

\bibitem[Mongus and Žalik(2012)]{Mongus2012tophat}
Mongus, D.; Žalik, B.
\newblock Parameter-free ground filtering of LiDAR data for automatic DTM
  generation.
\newblock {\em ISPRS~J. Photogramm. Remote Sens.} {\bf 2012},
  {\em 67},~1--12.


\bibitem[Borgefors(1986)]{BORGEFORS1986distance}
Borgefors, G.
\newblock Distance transformations in digital images.
\newblock {\em Comput. Vis. Graph. Image Process.} {\bf 1986},
  {\em 34},~344--371.


\bibitem[Powell(1964)]{powell1964efficient}
Powell, M.J.
\newblock An efficient method for finding the minimum of a function of several
  variables without calculating derivatives.
\newblock {\em Comput. J.} {\bf 1964}, {\em 7},~155--162.

\bibitem[Press \em{et~al.}(1989)Press, Flannery, Teukolsky, and
  Vetterling]{press1989numerical}
Press, W.H.; Flannery, B.P.; Teukolsky, S.A.; Vetterling, W.T.
\newblock \emph{Numerical Recipes}; Cambridge University Press: Cambridge, UK, 1989.

\bibitem[senseFly(2014 (accessed February 1, 2018))]{sullens}
\hl{senseFly. Oblique mapping of a village. }%Please provide link for this web reference.
\newblock URL: https://www.sensefly.com/education/datasets/.

\bibitem[M{\"u}ller and Seyfert(2000)]{atkis2000}
M{\"u}ller, W.; Seyfert, E.
\newblock Intelligent imagery system: A proposed approach.
\newblock {\em Int. Arch. Photogramm. Remote Sens.}
  {\bf 2000}, {\em 33},~710--717.

\bibitem[d’Angelo and Reinartz(2011)]{d2011semiglobal}
\textls[-20]{D’Angelo, P.; Reinartz, P.
\newblock Semiglobal Matching Results on the ISPRS Stereo Matching Benchmark; High-Resolution Earth Imaging for
  Geospatial Information;
\newblock  ISPRS Hannover Workshop: Hannover, Germany, 2011.}

\bibitem[Kurz \em{et~al.}(2014)Kurz, Rosenbaum, Meynberg, Mattyus, and
  Reinartz]{kurz2014performance}
Kurz, F.; Rosenbaum, D.; Meynberg, O.; Mattyus, G.; Reinartz, P.
\newblock Performance of a real-time sensor and processing system on a
  helicopter.
\newblock {\em ISPRS Int. Arch. Photogramm. Remote Sens. Spat. Inf. Sci.} {\bf 2014}, {\em 40},~189--193,
\newblock
  doi:{\changeurlcolor{black}\href{https://doi.org/10.5194/isprsarchives-XL-1-189-2014}{\detokenize{10.5194/isprsarchives-XL-1-189-2014}}}.

\bibitem[Jia \em{et~al.}(2014)Jia, Shelhamer, Donahue, Karayev, Long, Girshick,
...
\newblock {\em arXiv preprint arXiv:1408.5093} {\bf 2014}, doi:10.1145/2647868.2654889.
